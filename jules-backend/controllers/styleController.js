...
STYLE ADVICE MODE - MATCH GPT JULES PATTERN:
- Give strong, specific advice FIRST (like the job interview example)
- Then ask for context to refine it (e.g., "Tell me the job and company — I'll sharpen it for you")
- Be direct about what works and what doesn't
- Include specific items, brands, and fit guidance when relevant
- Keep responses short and punchy (2-3 short paragraphs max)
- Be bold, funny, sharp, fast
- Assume the user is smart and style-curious
- For packing advice, be specific about items and quantities
...
// Aggressive closer stripping for style advice
function stripStyleClosers(text) {
  if (!text) return text;
  ...
  const badClosers = [
    ...
    /\b(?:Now go make Europe your runway)\s*[.!?]*$/i,
    /\b(?:You're all set)\s*[.!?]*$/i,
    /\b(?:That's all you need)\s*[.!?]*$/i,
    /\b(?:You're ready)\s*[.!?]*$/i,
    /\b(?:You're good)\s*[.!?]*$/i,
...
exports.handleStyle = async (req, res) => {
  try {
    ...
    // Build system prompt for style
    const systemPrompt = getStyleSystemPrompt(userGender);
    ...
    // Aggressive closer stripping for style
    let cleanedReply = stripStyleClosers(reply);
    ...
    // Update user memory
    updateUserMemory(userId, { goals: "style advice" });
    ...
    console.error('Style handler error:', err);
